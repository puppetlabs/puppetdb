...
(def metadata-queries
  {:table_usage
   ...
   :string_fact_value_character_lengths
   "select percentile_cont(Array[0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30,
    0.35, 0.40, 0.45, 0.50, 0.55, 0.60,
    0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1])
    within group (order by length) quantiles 
    from
    (select char_length(value_string) as length from fact_values
   ...
   "select percentile_cont(Array[0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30,
    ...
    within group (order by length) quantiles 
    from
    (select char_length(value::text) as length from fact_values
   ...
   "select percentile_cont(Array[0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30,
    ...
    within group (order by length) quantiles 
    from
    (select char_length(metrics::text) as length from reports
   ...
   "select percentile_cont(Array[0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30,
    ...
    within group (order by length) quantiles 
    from
    (select char_length(logs::text) as length from reports
