...
;; REVIEW: only some queries are monitored (as with timeouts previously)
;; REVIEW: consider overall look, wrt logging, etc. when query is terminated
...
(defn- terminate-query
  ...
  ;; REVIEW: interruption - shutdown behavior, etc.
  (assert pg-pid)
  (try
    (when-not forget
...
(defn- monitor-queries [{:keys [exit queries selector] :as _monitor}
  ...
  ;; REVIEW: interruption - shutdown behavior, etc.
  ;;
  ;; Note that we depend on the fact that any new queries will wake us
...
;; REVIEW: expect to need this, e.g. for transient queries like
;; parent-check, but it isn't used right now because the current final
;; forget is sufficient (supersedes it).
