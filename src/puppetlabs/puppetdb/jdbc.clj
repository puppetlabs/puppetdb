	...
	(let [{:keys [entities transaction?]}
		...
		(if-let [con (sql/db-find-connection db)]
			...
			(with-open [con (sql/get-connection db)]
				(sql/db-do-prepared (sql/add-connection db con) transaction? sql-params {:multi? true})))))
...
(defn- multi-insert-helper
  ...
  [db stmts opts]
  (doall (map (fn [row] (sql/db-do-prepared-return-keys db false row opts)) stmts)))
...
(defn- insert-rows!
	...
	(let [{:keys [entities identifiers qualifier transaction?]}
				...
				sql-params (map (fn [row]
													...
													(insert-single-row-on-conflict table row entities opts)) rows)]
		(if-let [con (sql/db-find-connection db)]
