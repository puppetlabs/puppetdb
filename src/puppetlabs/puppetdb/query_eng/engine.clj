...
(def table-names #{"catalog_resources"
                   "certnames"
                   "edges"
                   "environments"
                   "fact_paths"
    ...
    (let [[subname & subexpr] expr
      ...
      (assoc logobj
             :projected-fields (mapv (fn [name]
                                       [name (projections name)])
                                     column-list)
             :where (when (seq subexpr)
                      (user-node->plan-node logobj (first subexpr)))))))
...
(defn user-node->plan-node
  ...
  (cm/match [node]
            ...
            [["in" column-s subquery-expr]]
            (map->InExpression
             (do
               (s/validate (s/conditional vector? [s/Str] :else s/Str) column-s)
               {:column (map #(get-in query-rec [:projections %])
                             (utils/vector-maybe column-s))
