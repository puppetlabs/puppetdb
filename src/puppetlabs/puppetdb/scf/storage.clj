...
(defn spawn-query-killer
  "Spawns a thread which will loop and wait for the gc-pid to get blocked
   ...
   from the gc-pid or the killer's pid. This should clear the way for GC to
   grab the lock it's requesting in the main GC thread. This loop repeats
   until the killer thread is interrupted or receives confirmation from the
   GC thread via the gc-finished? atom that the GC thread has dropped the
   partition."
  [db gc-pid gc-finished?]
  (let [{:keys [current_database current_user]}
        (first (jdbc/query-to-vec "select current_database(), current_user;"))

        kill-blocking-qs
        (fn [current_database current_user gc-pid]
          (jdbc/do-prepared
           (str "select pg_cancel_backend(pid)"
                " from pg_stat_activity"
                " where (datname = ? AND usename = ?)"
                "  and pid in (select unnest(pg_blocking_pids(?)))")
           [current_database current_user gc-pid]))]
    (Thread.
     #(with-noisy-failure
        (try
          ...
          ;; Binding *db* to the passed in db var will cause the killer thread
          ;; to get a new Hikari connection
          (binding [jdbc/*db* db]
            (while (not @gc-finished?)
              (kill-blocking-qs current_database
                                current_user
                                gc-pid)
              (Thread/sleep 1000)))
...
(def gc-query-killer-timeout-ms
  (env-config-for-db-ulong "PDB_GC_QUERY_KILLER_TIMEOUT_MS"
                           (* 10 60 1000)))

(defn drop-one-partition [drop-one candidate db]
  (if (pos? gc-query-killer-timeout-ms)
    (let [gc-pid (-> "select pg_backend_pid();"
                     jdbc/query-to-vec
                     first
                     :pg_backend_pid)
          gc-finished? (atom false)
          query-killer (spawn-query-killer db gc-pid gc-finished?)]
      (try
        (.start query-killer)
        (drop-one candidate)
        (finally
          (reset! gc-finished? true)
          ;; make sure the GC killer thread has broken out of its loop
          (.interrupt query-killer)
          (.join query-killer gc-query-killer-timeout-ms)
          (when (.isAlive query-killer)
            ;; if the join above timed out and the killer thread is still alive
            ;; log an ERROR because it could indicate a memory leak
            (log/error "Unable to clean up the gc killer thread.
                        Please file a bug with the stack trace below: \n"
                       (apply str (interpose "\n" (.getStackTrace query-killer))))))))
    (drop-one candidate)))
