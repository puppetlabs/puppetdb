...
(def json-primitive-schema (s/either s/String s/Number pls/SchemaBoolean))

(def resource-schema
  (merge resource-ref-schema
         {(s/optional-key :exported) pls/SchemaBoolean
          (s/optional-key :file) s/String
          (s/optional-key :line) s/Int
          (s/optional-key :tags) #{s/String}
          (s/optional-key :aliases)#{s/String}
          (s/optional-key :parameters) {s/Keyword (s/either json-primitive-schema
...
(def catalog-schema
  {:api-version s/Int
   ...
   :edges #{edge-schema}
   (s/optional-key :transaction-uuid) (s/maybe s/String)})

(def fact-map-schema
  {s/String json-primitive-schema})
...
(def resource-ref->hash {resource-ref-schema s/String})

(def edge-db-schema
  #{[(s/one s/String "source hash")
     (s/one s/String "target hash")
     (s/one s/String "relationship type")]})
...
(s/defn catalog-resources :- {resource-ref-schema s/String}
...
(s/defn insert-catalog-resources!
  ...
  [catalog-id :- s/Number
   refs-to-hashes :- {resource-ref-schema s/String}
   refs-to-resources :- resource-ref->resource-schema]
  (fn [refs-to-insert]
...
(s/defn update-catalog-resources!
  ...
  [catalog-id :- s/Number
   refs-to-hashes :- {resource-ref-schema s/String}
   old-resources :- {resource-ref-schema s/String}]
...
(s/defn update-catalog-hash-miss
  ...
  [catalog-id :- s/Number
   ...
   catalog :- catalog-schema
   refs-to-hashes :- {resource-ref-schema s/String}
   catalog-hash-debug-dir :- (s/maybe s/String)
   timestamp :- pls/Timestamp]
