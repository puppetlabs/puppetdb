...
(defn validate-limit
  "Validates that the limit string is a non-negative integer. Returns the integer
  ...
  (let [l (parse-int limit)]
    (if ((some-fn nil? neg? zero?) l)
      (throw (IllegalArgumentException.
               (format "Illegal value '%s' for :limit; expected a non-negative integer." limit)))
      l)))
...
(defn parse-limit
  ...
  Throws an exception if the provided limit is not a non-negative integer."
  [paging-options]
  (update-in paging-options [:limit] #(if (nil? %) % (validate-limit %))))
