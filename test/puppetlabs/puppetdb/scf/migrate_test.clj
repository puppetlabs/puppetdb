...
(deftest migration-73-schema-diff
  ...
  (let [before-migration (schema-info-map *db*)
    ...
    (is (= {:index-diff (into [{:left-only {:schema "public",
                                            :table "resource_events",
                              ...
                              (map (fn [part-name]
                                     [{:left-only nil,
                                       :right-only {:schema "public",
                                                    :table (str "resource_events_" part-name)
            ...
            :constraint-diff (into
                              ...
                              (map (fn [date-of-week]
                                     (let [part-name (partitioning/iso-week-year-suffix date-of-week)
                                       ...
                                       [{:left-only nil,
                                         :right-only {:constraint_name (format "(((\"timestamp\" >= '%s'::timestamp without time zone) AND (\"timestamp\" < '%s'::timestamp without time zone)))"
