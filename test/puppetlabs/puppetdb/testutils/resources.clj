   ...
   (with-transacted-connection *db*
     ...
     (sql/insert-records
       ...
       {:id 1 :hash "foo" :api_version 1 :catalog_version "12"
        :certname "one.local" :environment_id (when environment? (ensure-environment "DEV"))}
     ...
     (sql/insert-records :catalog_resources
                         {:catalog_id 1 :resource "1" :type "File" :title "/etc/passwd"
                          :exported false :tags (to-jdbc-varchar-array ["one" "two"])}
                         {:catalog_id 1 :resource "2" :type "Notify" :title "hello"
                          :exported false :tags (to-jdbc-varchar-array [])}
                         {:catalog_id 2 :resource "1" :type "File" :title "/etc/passwd"
                          :exported false :tags (to-jdbc-varchar-array ["one" "two"])}
                         {:catalog_id 2 :resource "2" :type "Notify" :title "hello"
                          :exported true :file "/foo/bar" :line 22 :tags (to-jdbc-varchar-array [])}))
