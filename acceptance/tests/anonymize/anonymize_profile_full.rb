test_name "anonymize tool - with full anonymization" do
  ...
  step "anonymize the data with profile 'full'" do
    on database, "#{sbin_loc}/puppetdb-anonymize --infile #{export_file1} --outfile #{anon_file} --profile full"
