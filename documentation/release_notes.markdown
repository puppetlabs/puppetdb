...
* For the best-possible performance and scaling capacity, we recommend
  the latest version of PostgreSQL (9.3 or higher) with the
  [`pg_trgm`][pg_trgm] extension, as explained
  [here][configure_postgres].  We have officially deprecated
  PostgreSQL versions earlier than 9.2, and HSQLDB is only recommended
  for local development because it has a number of scaling and
  operational issues.
