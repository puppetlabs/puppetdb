...
Justin Holguin, Ken Barber, Kylo Ginsberg, Rob Braden, Russell Sim, Ryan Senior and Wyatt Alt.

#### Changes to Endpoints

* [/v4/factsets](./api/query/v4/factsets)

    This endpoint will now return a `producer-timestamp` field, representing
    the time at which the master submits the replace-facts command to PuppetDB.
    Ultimately this timestamp will be produced by the agent to aid in
    timekeeping across multiple PuppetDBs. Until then it serves the same
    function as the existing `timestamp` field.

* [/v4/catalogs](./api/query/v4/catalogs)
j
    This endpoint will now return a "producer-timestamp" field, representing
    the time at which the master submits the replace-catalogs command to PuppetDB.
    Ultimately this timestamp will be produced by the agent to aid in
    timekeeping across multiple PuppetDBs. Until then it serves the same
    function as the existing `timestamp` field.
...
* [PDB-812](https://tickets.puppetlabs.com/browse/PDB-812) Collapse the produce-body construct from all the HTTP endpoints

    This patch consolidates produce-body functions for all streaming endpoints to a
    new function in query-eng.clj

* [PDB-843](https://tickets.puppetlabs.com/browse/PDB-843) Rationalize PDB namespaces
    ...
    This also moves the libraries in com.puppetlabs.* into puppetlabs.puppetdb
    to avoid collisions with any other project. These files were designed to be
    pulled out of PuppetDB eventually, but for now we've moved them all into the same
    namespace.

    We also have re-aligned our tests to be more standard, adopting the
    namespace-test convention. This aids with editors, as now we can switch easily between
    src and test.
    ...
    entry point. Bootstrap.cfg, and logback.xml have been changed also.
    ...
    For backwards compatibility, we've kept the old namespace pinned for JMX &
    ActiveMQ queues, since these are still required for our stable API commitments.
    In the future we'll have to fix this.

    The unused concurrent.clj code has been removed.
...
* [PDB-844](https://tickets.puppetlabs.com/browse/PDB-844) Format all clojure code with clojure-mode formatting

    This patch formats our clojure codebase with emacs clojure-mode.

* [PDB-813](https://tickets.puppetlabs.com/browse/PDB-813) Upgrade trapperkepper and move certificate-whitelist
...
* [PDB-489](https://tickets.puppetlabs.com/browse/PDB-489) Return producer timestamp in factsets and catalogs
...
* [PDB-882](https://tickets.puppetlabs.com/browse/PDB-882) Set up basic facts performance testing

    This introduces our first pass at performance testing, in particular we have
    targetted loading structured facts.

    The benchmark tool is now capable of using a series of provided fact JSON
    files,
    so we can use that tool for this purpose. We've also provided a set of basic
    seed files for these tests.

    The tooling: `rake test:beaker` has now become `rake beaker:acceptance` and
    performance tests are started using `rake beaker:performance` instead.

    The testing introduces a new host configuration specific to this work, in
    particular
    we are using a single c3.xlarge instance. This is isolated from the acceptance
    testing
    one so we can tune this on its own.

    This also fixs a number of side issues:

    * benchmark -h was broken, now fixed
    * The -rp switch no longer worked, this has no become -r
    * Some of the client based stuff was strewn around our code base, its now
    centralized in puppetlabs.puppetdb.client
...
* Update "sles-release" dependency to "distribution-release" virtual package

    The "sles-release" package only exists SLES. distribution-release
    is a virtual package that will install sles-release on SLES, or
    OpenSUSE-release on OpenSUSE.

* Switch to using c3.large testing instances
