...
-  Support for ActiveMQ has been completely removed, meaning that
   ...
   when upgrading from versions older than 4.3.0, but because PuppetDB's
   [upgrade policy][upgrade_policy] forbids skipping major versions,
   this should not present a problem, because any version 5 release 
   performs the conversion at startup.
...
raised an exception. Because the Puppet `http_pool` does not raise 503 
...
* Consider reports as part of node expiration, and don't expire nodes that have had a report because the currently configured `node-ttl`. ([PDB-1638](https://tickets.puppetlabs.com/browse/PDB-1638))
