...
layout: default
canonical: "/pe-puppetdb/latest/api/query/v4/reports.html"
---

[curl]: /puppetdb/latest/api/query/curl.html#using-curl-from-localhost-non-sslhttp
[operator]: /puppetdb/latest/api/query/v4/operators.html
[events]: /puppetdb/latest/api/query/v4/events.html
[paging]: /puppetdb/latest/api/query/v4/paging.html
[statuses]: /puppet/latest/reference/format_report.html#puppettransactionreport
[query]: /puppetdb/latest/api/query/v4/query.html
...
**Note on resources versus resource-events**
...
In the `resource_event` and `resources` schema above, `containment_path`, `new_value`, `old_value`, `property`, `file`, `line`, `status`, and `message` may all be null.

**Note on querying resource-events, resources, metrics, and logs**

The `reports` endpoint does not support querying on the value of `resource_events`, `resources`, `logs`,
...
Query for all reports:
    ...
    [ {
      ...
      "resources" : {
        "href": "/pdb/query/v4/reports/32c821673e647b0650717db467abc51d9949fd9a/resources",
        "data": [ {
          "file" : "/home/wyatt/.puppet/manifests/site.pp",
          "line" : 7,
          "resource_title" : "hiloo",
...
## `GET /pdb/query/v4/reports/<HASH>/resources`

This will return all resources for a particular report, designated by its unique hash.
